...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card text-card-foreground flex flex-col gap-8 rounded-[2rem] border-4 border-transparent py-8 shadow-xl hover:shadow-2xl transition-all duration-200",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("leading-tight font-bold text-3xl font-[family-name:var(--font-fredoka)]", className)}
      {...props}
    />
  )
}
